...
CLI 会通过本地 `u2cli` server 处理请求。普通命令会在需要时自动启动 server，也可以手动管理：
...
server 内部维护设备注册表，并按序列号和端口复用已经建立的设备连接。对于连续执行多条 CLI 命令的场景，这样可以让设备只连接一次，后续都交给 server 处理，避免每条命令重复连接同一台设备。
